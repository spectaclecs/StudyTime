...
class Window(FluentWindow):
    def __init__(self):
        ...
        self.table.setHorizontalHeaderLabels(['开始时间', '结束时间', '时长'])
        # self.table.horizontalHeader().setSectionResizeMode(QHeaderView.)
        # self.table.resizeColumnsToContents()

        layout.addWidget(self.start_button, 0, 2, 1, 2)
        layout.addWidget(self.stop_button, 1, 2, 1, 2)
        layout.addWidget(self.lcd, 2, 0, 2, 6)
        layout.addWidget(self.table, 4, 1, 2, 4)
...
def click_start(timer: QtCore.QTimer):
    ...
    print('start time:', start_time.toString('hh:mm:ss'))
...
def refresh_time(window: Window, lcd: QLCDNumber):
    window.time = window.time.addMSecs(10)
    if window.time.msec() // 10 % 10 == 0:
